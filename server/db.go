...
func dbFollow(ctx context.Context, uid, toUID int64) error {
	...
	if err != nil {
		excLog.Printf("ctx %v add user_follow uid %v to_uid %v err %v", ctx, uid, toUID, err)
		return err
	}
	err = tx.Create(follower).Error
	if err != nil {
		excLog.Printf("ctx %v add user_follower uid %v to_uid %v err %v", ctx, toUID, uid, err)
		return err
	}
	err = tx.Set("gorm:insert_option", "ON DUPLICATE key update follower_count = follower_count + 1").Create(&followCount).Error
	if err != nil {
		excLog.Printf("ctx %v add user_follow_count uid %v err %v", ctx, uid, err)
		return err
	}
	err = tx.Set("gorm:insert_option", "ON DUPLICATE key update follower_count = follower_count + 1").Create(&followerCount).Error
	if err != nil {
		excLog.Printf("ctx %v add user_follower_count uid %v err %v", ctx, toUID, err)
		return err
	}
	tx.Commit()
	return nil
...
func dbFollowTopic(ctx context.Context, uid, topicID int64) error {
	...
	if err != nil {
		excLog.Printf("ctx %v create followtopic uid %v topicid %v err %v", ctx, uid, topicID, err)
		return err
	}
	err = tx.Set("gorm:insert_option", "ON DUPLICATE key update follow_count = follow_count + 1").Create(&followTopicCount).Error
	if err != nil {
		excLog.Printf("ctx add followtopiccnt uid %v err %v", uid, err)
		return err
	}
	tx.Commit()
	return nil
...
func dbUnfollow(ctx context.Context, uid, toUID int64) error {
	...
	if err != nil {
		excLog.Printf("ctx %v delete user_follow uid %v to_uid %v err %v", ctx, uid, toUID, err)
		return err
	}
	err = dbCli.Where("uid = ? and follower_uid = ?", toUID, uid).Delete(&Follower{}).Error
	if err != nil {
		excLog.Printf("ctx %v delete user_follower uid %v to_uid %v err %v", ctx, toUID, uid, err)
		return err
	}
	err = dbCli.Model(&followCount).Where("uid = ? and follow_count > 0", uid).Update("follower_count", gorm.Expr("follow_count-1")).Error
	if err != nil {
		excLog.Printf("ctx %v delete user_follow_count uid %v err %v", ctx, uid, err)
		return err
	}
	...
	if err != nil {
		excLog.Printf("ctx %v delete user_follower_count uid %v err %v", ctx, toUID, err)
		return err
	}
	tx.Commit()
	return nil
...
func dbUnfollowTopic(ctx context.Context, uid, topicID int64) error {
	...
	if err != nil {
		excLog.Printf("ctx %v delete followtopic uid %v topicid %v err %v", ctx, uid, topicID, err)
		return err
	}
	err = dbCli.Model(&followTopicCount).Where("uid = ? and follow_count > 0", uid).Update("follow_count", gorm.Expr("follow_count - 1")).Error
	if err != nil {
		excLog.Printf("ctx %v delete followtopiccount uid %v err %v", ctx, uid, err)
		return err
	}
	tx.Commit()
	return nil
...
func dbGetFollowCount(ctx context.Context, uid int64) (int64, int64, error) {
	...
	if err != nil {
		excLog.Printf("ctx %v dbGetFollowCount uid %v err %v", ctx, uid, err)
		return 0, 0, err
	}
...
func dbGetFollowTopicCount(ctx context.Context, uid int64) (int64, error) {
	...
	if err != nil {
		excLog.Printf("ctx %v dbGetFollowTopicCount uid %v err %v", ctx, uid, err)
		return 0, err
	}
...
func dbGetFollow(ctx context.Context, uid int64) ([]int64, map[int64]int64, error) {
	...
	if err != nil {
		excLog.Printf("ctx %v db get user follow uid %v err %v", ctx, uid, err)
		return nil, nil, err
	}
...
func dbGetFollower(ctx context.Context, uid int64) ([]int64, map[int64]int64, error) {
	...
	if err != nil {
		excLog.Printf("ctx %v db get user follower uid %v err %v", ctx, uid, err)
		return nil, nil, err
	}
...
func dbGetFollowTopic(ctx context.Context, uid int64) ([]int64, map[int64]int64, error) {
	...
	if err != nil {
		excLog.Printf("ctx %v dbGetFollowTopic uid %v err %v", ctx, uid, err)
		return nil, nil, err
	}
